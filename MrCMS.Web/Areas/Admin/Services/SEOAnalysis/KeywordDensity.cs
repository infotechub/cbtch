...
namespace MrCMS.Web.Areas.Admin.Services.SEOAnalysis
{
    public class KeywordDensity : BaseSEOAnalysisFacetProvider
    {
        public override IEnumerable<SEOAnalysisFacet> GetFacets(Webpage webpage, HtmlNode document, string analysisTerm)
        {
            ...
            int instances = Regex.Matches(text, analysisTerm, RegexOptions.IgnoreCase).Count;

            string[] strings = text.Split(new[] { " " }, StringSplitOptions.RemoveEmptyEntries);
            int words = strings.Count();
            if (words == 0)
                yield break;
            int termWordCount = analysisTerm.Split(new[] { " " }, StringSplitOptions.RemoveEmptyEntries).Count();

            decimal density = ((decimal)instances / (words - (termWordCount * (termWordCount - 1)))) * 100m;
