...
namespace MrCMS.Web.Areas.Admin.Services
{
    public class PageDefaultsAdminService : IPageDefaultsAdminService
    {
        ...
        {
            string appName = MrCMSApp.AppWebpages[key];
        ...
        {
            int? layoutId = Settings.GetLayoutId(key);
            if (!layoutId.HasValue || layoutOptions.All(item => item.Value != layoutId.ToString()))
            {
                ...
                Layout systemDefaultLayout = _session.Get<Layout>(siteSettings.DefaultLayoutId);
