...
namespace MrCMS.Web.Areas.Admin.Services
{
    public class FormAdminService : IFormAdminService
    {
        ...
        public byte[] ExportFormData(Webpage webpage)
        {
            ...
            List<string> headers = GetHeadersForExport(webpage).ToList();
            stringBuilder.AppendLine(string.Join(",", headers.Select(FormatField)));

            Dictionary<int, List<string>> formDataForExport = GetFormDataForExport(webpage);
            foreach (KeyValuePair<int, List<string>> data in formDataForExport)
            {
        ...
        public PostingsModel GetFormPostings(Webpage webpage, int page, string search)
        {
            ...
            IQueryOver<FormPosting, FormPosting> query =
