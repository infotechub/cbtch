...
namespace MrCMS.Web.Areas.Admin.Services
{
    public class MediaSelectorService : IMediaSelectorService
    {
        ...
        {
            IQueryOver<MediaFile, MediaFile> queryOver = _session.QueryOver<MediaFile>().Where(file => file.Site.Id == _site.Id);
            if (searchQuery.CategoryId.HasValue)
                queryOver = queryOver.Where(file => file.MediaCategory.Id == searchQuery.CategoryId);
            if (!string.IsNullOrWhiteSpace(searchQuery.Query))
            {
                string term = searchQuery.Query.Trim();
                queryOver =
                    queryOver.Where(
                        file =>
        ...
        {
            string fileUrl = _fileService.GetFileUrl(value);
        ...
        {
            MediaFile mediaFile = GetImage(url);
        ...
        {
            MediaFile mediaFile = GetImage(updateMediaParams.Url);
            if (mediaFile == null)
                return false;
