        ...
        public override object BindModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
        {
            string typeName =
                GetValueFromContext(controllerContext, "type").Split(new[] { "-" }, StringSplitOptions.RemoveEmptyEntries)
                    [0];
            Type entityType = TypeHelper.MappedClasses.FirstOrDefault(type => type.FullName == typeName);

            if (entityType != null && entityType.HasDefaultConstructor())
            {
                Webpage bindModel = Activator.CreateInstance(entityType) as Webpage;

                string parentId = GetValueFromContext(controllerContext, "parentId");
                int id;
