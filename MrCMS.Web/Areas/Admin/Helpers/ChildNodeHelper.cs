...
namespace MrCMS.Web.Areas.Admin.Helpers
{
    public static class ChildNodeHelper
    {
        ...
        {
            HtmlNode thisNode = node.ChildNodesRecursive().FirstOrDefault(htmlNode => htmlNode.Name == nodeType);
        ...
        {
            List<string> text = node.ChildNodesRecursive().Select(htmlNode => htmlNode.InnerText).ToList();
        ...
        {
            HtmlNodeCollection htmlNodeCollection = node.ChildNodes;
