...
namespace MrCMS.Web.Apps.Core.SitemapGeneration
{
    public class GenerateSitemapForTextPage : SitemapGenerationInfo<TextPage>
    {
        ...
        {
            System.DateTime? publishOn = webpage.PublishOn;
            if (!publishOn.HasValue)
                return;
            string content = webpage.PublishOn.Value.SitemapDateString();
            XElement urlNode = new XElement(SitemapService.RootNamespace + "url",
