...
namespace MrCMS.Indexing.Management
{
    ...
    public abstract class IndexDefinition<T> : IndexDefinition where T : SystemEntity
    {
        ...
        public Document Convert(T entity)
        {
            ...
            foreach (IFieldable additionalField in GetAdditionalFields(entity))
        ...
        {
            List<FieldDefinition<T>> fieldDefinitions = GetNewDefinitionList();
            fieldDefinitions.AddRange(Definitions);
            List<Dictionary<T, List<AbstractField>>> list = fieldDefinitions.Select(fieldDefinition => fieldDefinition.GetFields(entities)).ToList();
            Dictionary<T, IEnumerable<IFieldable>> additionalFields = GetAdditionalFields(entities);
            List<Document> documents = new List<Document>();
            foreach (T entity in entities)
            {
                Document document = new Document();
                foreach (Dictionary<T, List<AbstractField>> fieldInfo in list)
                {
                ...
                {
                    foreach (IFieldable additionalField in additionalFields[entity])
                    {
                        document.Add(additionalField);
                    }
                }
        ...
        {
            Dictionary<Type, List<Func<SystemEntity, IEnumerable<LuceneAction>>>> actionsDictionary = new Dictionary<Type, List<Func<SystemEntity, IEnumerable<LuceneAction>>>>
                                    {
                                        {
                                            typeof (T),
            ...
            foreach (IFieldDefinitionInfo fieldDefinitionInfo in DefinitionInfos)
            {
        ...
        {
            Dictionary<Type, Func<SystemEntity, IEnumerable<LuceneAction>>> relatedEntities = fieldDefinitionInfo.GetRelatedEntities();
            foreach (Type key in relatedEntities.Keys)
            {
        ...
        {
            Type indexDefinitionType = GetType();
            Dictionary<int, Dictionary<Type, IndexSearcher>> siteDictionary = IndexSearcherCache.Value;
            int siteId = _getLuceneIndexSearcher.SiteId;
            if (!siteDictionary.ContainsKey(siteId))
            {
                siteDictionary[siteId] = new Dictionary<Type, IndexSearcher>();
            }
            Dictionary<Type, IndexSearcher> dictionary = siteDictionary[siteId];
