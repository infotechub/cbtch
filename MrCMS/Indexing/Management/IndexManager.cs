...
namespace MrCMS.Indexing.Management
{
        ...
        where TDefinition : IndexDefinition<TEntity>
    {
        ...
        public IndexResult Update(TEntity entity)
        {
            return IndexResult.GetResult(() => Write(writer =>
            {
                using (IndexSearcher indexSearcher = new IndexSearcher(GetDirectory(_site), true))
                {
        ...
        {
            ICriteria criteria = _statelessSession.CreateCriteria(typeof(TEntity))
                .Add(Restrictions.Eq("IsDeleted", false));
            if (typeof(SiteEntity).IsAssignableFrom(typeof(TEntity)))
                criteria.Add(Restrictions.Eq("Site.Id", _site.Id));
            List<TEntity> entities = criteria.SetCacheable(true)
                .List<TEntity>().ToList();
            return IndexResult.GetResult(() =>
            {
                Write(writer => { }, true);
        ...
        {
            using (
                IndexWriter indexWriter = new IndexWriter(GetDirectory(_site), Definition.GetAnalyser(), recreateIndex,
