...
namespace MrCMS.Batching
{
    public class DefaultBatchJobExecutor : IBatchJobExecutor
    {
        ...
        {
            string message = string.Format("There is no executor for this job. To create one, implement {0}<{1}>",
                typeof(IBatchJobExecutor).FullName,
                batchJob.GetType().FullName);
            BatchJobExecutionResult batchJobExecutionResult = BatchJobExecutionResult.Failure(message);
