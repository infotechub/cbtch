    ...
    public class CacheManager : ICacheManager
    {
        ...
        public T Get<T>(string key, Func<T> func, TimeSpan time, CacheExpiryType cacheExpiryType)
        {
            ...
            if (o != null)
            {
                if (time > TimeSpan.Zero)
                {
                    DateTime absoluteExpiration = cacheExpiryType == CacheExpiryType.Absolute
                        ? DateTime.UtcNow.Add(time)
                        : Cache.NoAbsoluteExpiration;
                    TimeSpan slidingExpiration = cacheExpiryType == CacheExpiryType.Sliding
                        ? (time)
        ...
        public void Clear(string prefix = null)
        {
            ...
            else
            {
                ...
                string fullPrefix = InternalCachePrefix + prefix;
