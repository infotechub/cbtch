...
namespace MrCMS.Website
{
    public abstract class MrCMSPage<TModel> : WebViewPage<TModel>
    {
        ...
        public MvcHtmlString Editable<T>(T model, Expression<Func<T, string>> method, bool isHtml = false) where T : SystemEntity
        {
            ...
            System.Reflection.PropertyInfo propertyInfo = PropertyFinder.GetProperty(method);

            string value = Html.ParseShortcodes(method.Compile().Invoke(model)).ToHtmlString();

            string typeName = model.GetType().Name;

            if (EditingEnabled && propertyInfo != null)
            {
                TagBuilder tagBuilder = new TagBuilder(isHtml ? "div" : "span");
        ...
        public void RenderZone(string areaName, Webpage page = null, bool allowFrontEndEditing = true)
        {
            ...
            Entities.Documents.Layout.Layout currentLayout = GetCurrentLayout.Get(page);
            if (page != null && currentLayout != null)
            {
                bool allowEdit = EditingEnabled && allowFrontEndEditing;

                Entities.Documents.Layout.LayoutArea layoutArea = currentLayout.GetLayoutAreas().FirstOrDefault(area => area.AreaName == areaName);
                ...
                foreach (Entities.Widget.Widget widget in layoutArea.GetWidgets(page))
                {
                    if (allowEdit)
                        ViewContext.Writer.Write(
