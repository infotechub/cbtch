...
namespace MrCMS.Tasks
{
    public class TaskResetter : ITaskResetter
    {
        ...
        {
            System.Collections.Generic.IList<ScheduledTask> hungScheduledTasks = session.QueryOver<ScheduledTask>()
                .Where(
        ...
        {
            System.Collections.Generic.IList<QueuedTask> hungTasks = session.QueryOver<QueuedTask>()
                .Where(
                    task => task.Site.Id == _site.Id &&
                            (task.Status == TaskExecutionStatus.AwaitingExecution ||
                             task.Status == TaskExecutionStatus.Executing) &&
