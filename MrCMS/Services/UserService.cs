...
namespace MrCMS.Services
{
    public class UserService : IUserService
    {
        ...
        {
            return _session.QueryOver<User>().Where(x => x.IsActive == true).Cacheable().List<User>();


        }

        public IList<User> GetAllUsersByRole(UserRole roleid)
        {
            List<User> dlist =_session.QueryOver<User>().Where(x => x.IsActive).Cacheable().List<User>().Where(x=> x.Roles.Contains(roleid)).ToList<User>();
        ...
        public User GetUserByGUID(string id)
        {
            ...
            return _session.QueryOver<User>().Where(user => user.Guid == guid).Take(1).Cacheable().SingleOrDefault();
        }
    }
}