...
namespace MrCMS.Services
{
    public class EmailSender : IEmailSender, IDisposable
    {
        ...
        public void SendMailMessage(QueuedMessage queuedMessage)
        {
            try
            {
                MailMessage mailMessage = BuildMailMessage(queuedMessage);
        ...
        private static MailMessage BuildMailMessage(QueuedMessage queuedMessage)
        {
            ...
            string[] multipleToAddress = queuedMessage.ToAddress.Split(new char[] { ',',';' }, StringSplitOptions.RemoveEmptyEntries);
            if (multipleToAddress.Any()){
                foreach (string email in multipleToAddress)
                {
