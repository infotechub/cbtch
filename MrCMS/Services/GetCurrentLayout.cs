...
namespace MrCMS.Services
{
    public class GetCurrentLayout : IGetCurrentLayout
    {
        ...
        public Layout Get(Webpage webpage)
        {
            if (webpage != null)
            {
                Layout layout = GetPageTemplateLayout(webpage);
                if (layout != null)
                {
                    return layout;
                }
        ...
        {
            DocumentMetadata documentMetadata = webpage.GetMetadata();
            int? layoutId = _pageDefaultsSettings.GetLayoutId(documentMetadata.Type);
            if (layoutId.HasValue)
            {
                Layout layout = _session.Get<Layout>(layoutId);
                if (layout != null)
                {
                    return layout;
                }
