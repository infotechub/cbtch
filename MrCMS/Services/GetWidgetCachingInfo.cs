    ...
    public class GetWidgetCachingInfo : IGetWidgetCachingInfo
    {
        public CachingInfo Get(Widget widget)
        {
            using (MiniProfiler.Current.Step("Get caching info"))
            {
                ...
                OutputCacheableAttribute attribute = widget.GetType().GetCustomAttribute<OutputCacheableAttribute>();
                if (attribute == null)
                {
                    return CachingInfo.DoNotCache;
                }
                bool shouldCache = widget.Cache && widget.CacheLength > 0;
        ...
        {
            string cacheKey = "Widget." + widget.Id;
            if (attribute.PerPage)
            {
                Entities.Documents.Web.Webpage page = CurrentRequestData.CurrentPage;
                if (page != null)
                {
