...
namespace MrCMS.Services.ImportExport
{
    public class ImportExportManager : IImportExportManager
    {
        ...
        {
            ExcelPackage spreadsheet = new ExcelPackage(file);

            Dictionary<string, List<string>> parseErrors;
            List<DocumentImportDTO> items = GetDocumentsFromSpreadSheet(spreadsheet, out parseErrors);
            if (parseErrors.Any())
                return ImportDocumentsResult.Failure(parseErrors);
            Dictionary<string, List<string>> businessLogicErrors = _importDocumentsValidationService.ValidateBusinessLogic(items);
            if (businessLogicErrors.Any())
                return ImportDocumentsResult.Failure(businessLogicErrors);
            Batch batch = _importDocumentService.CreateBatch(items, autoStart);
        ...
        {
            List<Webpage> webpages = _documentService.GetAllDocuments<Webpage>().ToList();
            ExcelPackage package = _exportDocumentsService.GetExportExcelPackage(webpages);
