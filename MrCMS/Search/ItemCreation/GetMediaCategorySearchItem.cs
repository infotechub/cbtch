...
namespace MrCMS.Search.ItemCreation
{
    public class GetMediaCategorySearchItem : GetUniversalSearchItemBase<MediaCategory>
    {
        ...
        {
            HashSet<string> primarySearchTerms = _getMediaCategorySearchTerms.SelectMany(terms => terms.GetPrimary(entity)).ToHashSet();
            HashSet<string> secondarySearchTerms = _getMediaCategorySearchTerms.SelectMany(terms => terms.GetSecondary(entity)).ToHashSet();
        ...
        public override HashSet<UniversalSearchItem> GetSearchItems(HashSet<MediaCategory> entities)
        {
            ...
            foreach (IGetMediaCategorySearchTerms getMediaCategorySearchTerms in _getMediaCategorySearchTerms)
            {
                Dictionary<MediaCategory, HashSet<string>> terms = getMediaCategorySearchTerms.GetPrimary(entities);
                foreach (MediaCategory key in terms.Keys)
                {
            ...
            foreach (IGetMediaCategorySearchTerms getMediaCategorySearchTerms in _getMediaCategorySearchTerms)
            {
                Dictionary<MediaCategory, HashSet<string>> terms = getMediaCategorySearchTerms.GetSecondary(entities);
                foreach (MediaCategory key in terms.Keys)
                {
