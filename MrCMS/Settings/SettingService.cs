...
namespace MrCMS.Settings
{
    ...
    public partial class SettingService : ISettingService
    {
        ...
        public virtual Setting GetSettingByKey(string key)
        {
            ...
            IDictionary<string, KeyValuePair<int, string>> settings = GetAllSettings();
            if (settings.ContainsKey(key))
            {
                int id = settings[key].Key;
        ...
        private Dictionary<string, KeyValuePair<int, string>> GetSettingsDictionary()
        {
            ...
            foreach (Setting s in AllSettings.Where(setting => setting.Site != null && setting.Site.Id == _site.Id))
            {
        ...
        private IList<Setting> AllSettings
        {
            get
            {
                IList<Setting> allSettings = _allSettings = _allSettings ?? GetAllSettingsFromDb();
                if (!allSettings.Any())
                {
                    try
                    {
