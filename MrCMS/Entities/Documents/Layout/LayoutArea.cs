...
namespace MrCMS.Entities.Documents.Layout
{
    public class LayoutArea : SiteEntity
    {
        ...
        {
            List<Widget.Widget> widgets = Widgets.Where(widget => widget.Webpage == null).ToList();
            ...
            if (webpage != null)
            {
                ...
                List<Widget.Widget> widgetsToRemove = new List<Widget.Widget>();

                if (!showHidden)
                    widgetsToRemove.AddRange(widgets.Where(webpage.IsHidden));

                foreach (Widget.Widget widget in widgetsToRemove)
                    widgets.Remove(widget);

                if (webpage.PageWidgetSorts != null)
                {
                    List<PageWidgetSort> pageWidgetSorts =
