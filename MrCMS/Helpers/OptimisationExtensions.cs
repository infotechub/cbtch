...
namespace MrCMS.Helpers
{
    public static class OptimisationExtensions
    {
        ...
        {
            WebPageBase webPage = helper.ViewDataContainer as WebPageBase;
            string virtualPath = webPage == null ? string.Empty : webPage.VirtualPath;
        ...
        {
            foreach (string script in MrCMSApplication.GetAll<IAppScriptList>().SelectMany(appScriptList => appScriptList.UIScripts))
            {
        ...
        {
            foreach (string script in MrCMSApplication.GetAll<IAppScriptList>().SelectMany(appScriptList => appScriptList.AdminScripts))
            {
                html.IncludeScript(script);
            }
        }
        ...
        {
            foreach (string script in MrCMSApplication.GetAll<IAppStylesheetList>().SelectMany(appScriptList => appScriptList.UIStylesheets))
            {
        ...
        {
            foreach (string script in MrCMSApplication.GetAll<IAppStylesheetList>().SelectMany(appScriptList => appScriptList.AdminStylesheets))
            {
                html.IncludeCss(script);
            }
        }
