...
namespace MrCMS.Helpers
{
    public class UniquePageHelper
    {
        ...
        {
            IUniquePageService service = MrCMSApplication.Get<IUniquePageService>();

            T processPage = service.GetUniquePage<T>();
            string url = processPage != null ? selector(processPage) : "/";
            if (queryString != null && processPage != null)
            {
                RouteValueDictionary dictionary = new RouteValueDictionary(queryString);
                url += string.Format("?{0}",
                                     string.Join("&",
                                                 dictionary.Select(
