...
namespace MrCMS.Helpers
{
    public static class MrCMSHtmlHelper
    {
                                                     ...
                                                     string text = null)
        {
            ...
            string checkbox = (CheckBoxHelper(htmlHelper, metadata, htmlFieldName, htmlHelper.ViewData.Model != null ? expression.Compile()(htmlHelper.ViewData.Model) : (bool?)null, AnonymousObjectToHtmlAttributes(checkboxAttributes)).ToHtmlString());
        ...
        {
            ViewContext viewContext = new ViewContext(controller.ControllerContext, new FakeView(), controller.ViewData, controller.TempData, TextWriter.Null);
        ...
        {
            foreach (KeyValuePair<string, object> key in additions)
        ...
        {
            TagBuilder tagBulder = new TagBuilder("div");
            tagBulder.AddCssClass("alert alert-" + alertType);

            if (!string.IsNullOrEmpty(boldText))
            {
                TagBuilder strongText = new TagBuilder("strong");
