...
namespace MrCMS.Shortcodes
{
    public class FormShortcodeParser : IShortcodeParser
    {
        ...
        public string Parse(HtmlHelper htmlHelper, string current)
        {
            ...
            List<string> errors = htmlHelper.ViewContext.TempData["form-submitted-message"] as List<string>;
            NameValueCollection data = htmlHelper.ViewContext.TempData["form-data"] as NameValueCollection;
            FormSubmittedStatus status = new FormSubmittedStatus(submitted, errors, data);
            current = OtherPageFormMatch.Replace(current,
                match =>
                {
                    int pageId = Convert.ToInt32(
                        match.Value.Replace("[", "").Replace("]", "").Split('-')[1]);
                    Webpage document = _documentService.GetDocument<Webpage>(pageId);
